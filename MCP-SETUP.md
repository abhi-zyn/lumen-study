# Automating StudyForge from Claude (free plan)
...
> "Look at my last two weeks in StudyForge, grade it out of 10 and save the review as a note."
...
The function is already deployed on the live project, so you only need the secrets:

```bash
# secrets (or set them in the dashboard: Project Settings → Edge Functions → Secrets)
...
In claude.ai → **Settings → Connectors → Add custom connector** → paste that URL → Add. Open a new chat, click the tools icon, and enable the connector. Test with: "Add a study block: 2026-09-20, Physics, Thermodynamics, 45 minutes." Then open StudyForge → Planner.

JWT verification is off at the gateway because Claude can't send a Supabase JWT; the `?key=` secret is the authentication instead. **Treat that URL like a password** — anyone with it can write to your data. Rotate by setting a new `MCP_SECRET` and re-adding the connector.
...
2. **Drive → StudyForge.** Share the file as *Anyone with the link*, then tell Claude: "import_pdf_from_url this link." The connector converts the share link to a direct download, verifies it really is a PDF, uploads it to your private `textbooks` bucket and adds the book row.
...
- Claude's chat history and in-app comments remain inaccessible; anything you want to keep lives in StudyForge, written through `add_note`.
